...
export default function Agenda() {
  ...
  // Gera os dias da semana atual baseado no dia selecionado
  ...
  // Orçamentos agendados para o dia selecionado
  const agendadosDia = orcamentos.filter(o => {
    if (!o.data_agendamento) return false
    return isMesmoDia(new Date(o.data_agendamento), diaSelecionado)
  }).sort((a, b) => new Date(a.data_agendamento).getTime() - new Date(b.data_agendamento).getTime())

  // Orçamentos aprovados sem agendamento
  const semAgendamento = orcamentos.filter(o => !o.data_agendamento)

  // Todos os dias que têm agendamento nessa semana
  const diasComAgendamento = diasSemana.map(dia => ({
    dia,
    count: orcamentos.filter(o => o.data_agendamento && isMesmoDia(new Date(o.data_agendamento), dia)).length
  }))
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              <div>
                ...
                <input type="date" value={dataInput} onChange={e => setDataInput(e.target.value)}
                  style={{ width: '100%', background: '#0f1117', border: '1px solid #2a2d3e', borderRadius: '10px', padding: '12px 16px', color: '#f1f5f9', fontSize: '14px', outline: 'none', boxSizing: 'border-box', fontFamily: "'DM Sans', sans-serif" }} />
              </div>
              <div>
                ...
                <input type="time" value={horaInput} onChange={e => setHoraInput(e.target.value)}
                  style={{ width: '100%', background: '#0f1117', border: '1px solid #2a2d3e', borderRadius: '10px', padding: '12px 16px', color: '#f1f5f9', fontSize: '14px', outline: 'none', boxSizing: 'border-box', fontFamily: "'DM Sans', sans-serif" }} />
              </div>
              <button onClick={salvarAgendamento} disabled={salvando} style={{
                background: salvando ? '#374151' : 'linear-gradient(135deg, #6366f1, #8b5cf6)',
                color: 'white', border: 'none', padding: '14px', borderRadius: '12px',
                fontSize: '14px', fontWeight: 700, cursor: salvando ? 'not-allowed' : 'pointer',
                fontFamily: "'DM Sans', sans-serif", marginTop: '4px'
              }}>{salvando ? 'Salvando...' : '✓ Confirmar agendamento'}</button>
            </div>
          </div>
        </div>
      )}
            ...
            <div style={{ background: '#16181f', border: '1px solid #1e2130', borderRadius: '16px', padding: '16px 20px', marginBottom: '20px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: '6px' }}>
                {diasComAgendamento.map(({ dia, count }, i) => {
                  ...
                  return (
                    <div key={i} onClick={() => setDiaSelecionado(new Date(dia))} style={{
                      display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px',
                      padding: '10px 4px', borderRadius: '12px', cursor: 'pointer',
                      background: isSelecionado ? 'linear-gradient(135deg, rgba(99,102,241,0.25), rgba(139,92,246,0.15))' : 'transparent',
                      border: isSelecionado ? '1px solid rgba(99,102,241,0.4)' : '1px solid transparent',
                      ...
                      {count > 0 && (
                        <div style={{ width: '6px', height: '6px', borderRadius: '50%', background: isSelecionado ? '#a5b4fc' : '#6366f1' }} />
                      )}
                      {count === 0 && <div style={{ width: '6px', height: '6px' }} />}
                    </div>
                  )
                })}
              </div>
            ...
            <div className="agenda-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: '20px' }}>
              ...
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '14px' }}>
                  <h2 style={{ fontFamily: "'Syne', sans-serif", fontSize: '16px', fontWeight: 700, margin: 0, color: '#f1f5f9', textTransform: 'capitalize' }}>
                    {formatarDataExtenso(diaSelecionado)}
                  </h2>
                  {isMesmoDia(diaSelecionado, hoje) && (
                    <span style={{ background: 'rgba(99,102,241,0.2)', color: '#a5b4fc', fontSize: '11px', fontWeight: 600, padding: '2px 10px', borderRadius: '20px', border: '1px solid rgba(99,102,241,0.3)' }}>Hoje</span>
                  )}
                </div>
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            ...
                            <div style={{ display: 'flex', gap: '8px' }}>
                              <button onClick={() => { setModalAgendamento(o); setDataInput(o.data_agendamento?.split('T')[0] || ''); setHoraInput(o.data_agendamento?.split('T')[1]?.slice(0,5) || '') }}
                                style={{ background: 'rgba(99,102,241,0.1)', border: '1px solid rgba(99,102,241,0.2)', color: '#a5b4fc', padding: '5px 12px', borderRadius: '8px', fontSize: '12px', cursor: 'pointer', fontFamily: "'DM Sans', sans-serif" }}>✏️ Editar</button>
                              <button onClick={() => removerAgendamento(o.id)}
                                style={{ background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.15)', color: '#f87171', padding: '5px 12px', borderRadius: '8px', fontSize: '12px', cursor: 'pointer', fontFamily: "'DM Sans', sans-serif" }}>✕</button>
                            </div>
                          </div>
                        </div>
                      </div>
              ...
              {/* Painel lateral - sem agendamento */}
              <div>
                <h2 style={{ fontFamily: "'Syne', sans-serif", fontSize: '15px', fontWeight: 700, margin: '0 0 14px', color: '#f1f5f9' }}>
                  ⏳ Aguardando agendamento
                  {semAgendamento.length > 0 && (
                    <span style={{ marginLeft: '8px', background: 'rgba(245,158,11,0.15)', color: '#fbbf24', fontSize: '11px', padding: '2px 8px', borderRadius: '20px', fontWeight: 600 }}>{semAgendamento.length}</span>
                  )}
                </h2>
